...
In order to blend the benefits of the CTMC based movement model with the closed-form, log-linear UD of the Langevin SDE model we propose a class of CTMC models for which one can obtain closed-form log-linear predictions of the UD from the fitted movement model parameters. Markov Chain Monte Carlo (MCMC) theory is used to verify that these specific models possess the desired form for their stationary distributions (e.g., \citealt{michelot2018linking, Blackwell2026a}) and we develop the HMM likelihood for fitting the model, specifically for normally distributed location error. To fit these models, we developed the {\tt R} package {\tt walk}\footnote{Available from: \url{https://dsjohnson.r-universe.dev/walk}} \citep{Johnson2026a} to estimate parameters of general CTMC models using the exact HMM likelihood \citep{mcclintock2020uncovering}. The process of fitting models and making UD inference is demonstrated with telemetry data from a false killer whale (\emph{Pseudorca crassidens}) population in the Hawaiian Islands. 
...
One area of probability oriented toward constructing Markov chains which have a specified stationary distribution is Markov Chain Monte Carlo (MCMC). So we MCMC theoretical results \citep{michelot2018linking,Blackwell2026a} to show that a CTMC movement model has a stationary distribution of log-linear form. To verify the closed-form stationary distribution we need only check that the MCMC reversibility condition is satisfied for a given $\bpi$, that is,
\[
\pi_iP_{ij} = \pi_jP_{ji},
\]
...
While overarching movement and space-use inference is also shared by Langevin diffusion movement models \citep{michelot2018langevin}, there are benefits to using the CTMC models proposed here. First, although it can be computationally intensive, one can use the exact likelihood for the movement data. For Langevin diffusion inference an Euler or Ozaki time discretization approximation is needed within a pseudo-likelihood \citep{GloaguenEtAl2018a,michelot2018langevin} for parameter estimation. One might make the argument that the CTMC approach just exchanges discrete space for discrete time approximation. This may be true for constructed variables such as distance from land in the FKW example. However, many habitat variables are only available as cellular rasters, e.g., SST and SLA. Therefore, the Langevin pseudo-likelihoods must also spatially approximate the gradients of these variables as well. In addition, while the Langevin model can, in theory, handle uneven gaps in telemetry data, in practice, interpolation is necessary for model fitting. If there is no location error, then straight line interpolation may be sufficient, but location uncertainty generally implies a stochastic imputation method is needed \citep{michelot2018langevin, ScharfEtAl2017a}. The CTMC model presented here does not need data interpolation or specification of an imputation model.  Finally, the Langevin models are not well suited to use of categorical habitat characteristics \citep{michelot2018langevin} and a smoothing of indicator variables must be used. The CTMC model proposed here can use categorical variables just as easily as continuously valued variables.    

Given the apparent positive aspects of the closed-form CTMC approach, it behooves us to look at some potential downsides. The first issue is that CTMC models are built on a defined set of cells, if it is changed, for example, the resolution of a raster grid is increased, it is not apparent how the parameters relate to each other. If the grid is fixed based on habitat covariates, then this is probably not an issue, but if there is some flexibility in resolution of cells then some consideration is warranted. Although, we conjecture, there is a minimum cell size for which a researcher would be interested in assessing use and movement. For example, in the FKW example, determining use over a 9m (versus $\sim$9km) grid would be nonsensical and not necessary for scientific or management purposes. The second issue is that the TPM, $\bG(t)$, can become difficult to compute for very large study areas relative to cell resolution. Uniformization methods \citep{sherlock2021direct} and rate censoring as implemented in the {\tt walk} package provide ways to mitigate this issue. But for very large study areas and very fast moving species (i.e., high $\max(\Lambda_i)$) it can become burdensome. Alternative uniformization methods which calculate parameter gradients \citep{RuppEtAl2024a} may provide avenues to speed optimization. 

Ultimately, bridging the gap between fine-scale movement decisions and long-term space-use remains a central challenge in spatial ecology. By offering a computationally tractable, closed-form bridge between continuous-time movement and utilization distributions, the CTMC framework provides a unified approach to movement and space-use modeling. As telemetry datasets continue to grow in resolution and volume, methods that seamlessly blend rigorous movement theory with flexible population-level inference will be essential for management and conservation. The combination of CTMC models for individual movement and meta-analysis methods for population inference provides a theoretical and practical approach for transforming raw telemetry data into scientific inference to reach this goal.    

...
Herein we consider a general special case of the CTMC model used by \cite{wilsonestimating} for which we can obtain closed form predictions of the UD from the fitted model. The CTMC model has several advantages over the Langevin UD. First, there is no approximation necessary to calculate the likelihood for optimization. Second, continuous or discrete habitat covariates can be used for movement modeling and UD prediction. Finally, overall movement rate can be influenced by habitat, that is diffusion rate can be altered by habitat, not just advection. Moreover, because there is no approximation for the likelihood that error does not propagate to the UD prediction, so it is exact. To fit these model, we also developed the {\tt R} package {\tt walk}\footnote{Available from: \url{https://dsjohnson.r-universe.dev/walk}} that can fit general CTMC models including the more general forms requiring numerical calculation of the UD. 
...
\item $c_{ij} = c_{ji}$
\end{enumerate}
Thus using the CTMC limiting distribution result, the UD of the CTMC defined by $\lambda_{ij}$ is $u_i \propto \exp\{b_i-a_i\}$. There is another specific condition on $c_{ij}$ that also provides a similar result. If $c_{ij}$ is constructed as a difference between cell indexed terms, e.g., $c_{ij} = d_j - d_i$ where movement effects are modeled with relative local differences in cell effects, we can still evaluate the UD in the closed exponential form. We just need to evaluate the $R_m$ ratio,
\begin{equation}
\begin{aligned}
R_m &= \frac{\exp\{b_i-a_i\}\Lambda_i}{\alpha_i} \times \frac{\alpha_j}{\exp\{b_j-a_j\}\Lambda_j} \times \exp\{2d_i-2d_j\} \\
&= \frac{\exp\{b_i-a_i+2d_i\}\Lambda_i}{\alpha_i} \times \frac{\alpha_j}{\exp\{b_j-a_j+2d_j\}\Lambda_j}.
\end{aligned}
\end{equation}
Thus, for all local difference effects, we amplify the effect on the limiting distribution by 2, that is, 
\begin{equation}
\alpha_i \propto \exp\{b_i-a_i+2d_i\}\Lambda_i
\end{equation}
satisfies the $R_m\ge 1$ condition and is the limiting distribution. 

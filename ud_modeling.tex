...
{\Large \bf Modeling Animal Space-Use with Continuous-Time Markov Chains}
...
\noindent {\bf Abstract.}This happens last. \bigskip

\noindent {\bf Key words}: Some, Key, words, Go, Here

% \clearpage

\section{Introduction}

Any quick literature search will reveal that remote collection and analysis of animal locations, or telemetry data, have become a ubiquitous and important tool in the advancement of animal ecology. A perennial subject within animal telemetry analysis is the assessment of space-use. That is what aspects of habitat and movement control where an animal might occur on the landscape. The topic of animal space-use is often used in conjunction with the terms home range, resource selection, or utilization distribution. Here we are specifically concerned with the concept of utilization distributions (UD) as defined by \cite{powell2012home}, a probability distribution that describes an animal's location at any random time. 

Literature on UDs is generally divided into two types. The first type is what we term {\it empirical} UDs. Empirical UDs are constructed based on a summary of observed locations, for example, the traditional kernel density (see \citealt{Keating:2009hl}) or Brownian bridge estimators \citep{Horne:2007mw} of UDs are empirical in that they place positive mass over observed locations. If an animal does not visit a particular area there will be very little if any use estimated for that area depending on how close it is to any observed locations. However, absence of observed use may be a symptom of {\it start bias} \citep{whitehead2013inferring}. Start bias results from tag deployments that are not long enough for the animal to fully explore the space and habitat available to it over the course of study. Therefore, spatial use assessments based solely on the observed locations can be biased towards the release location of the animal. This is especially true in recent years where many locations can be obtained over a fine time-scale and thus produce highly autocorrelated locations. To match the increased technological sophistication of location devices, statistical methodology of empirical UD methods have increased to account for the high autocorrelation (e.g., \citealt{fleming2015rigorous, calabrese2016ctmm}). However, the most basic solution for reducing start bias is to increase data collection (i.e., tag deployment) interval length  \citep{Fieberg:2010fk}. This basic solution motivates the next type of UD, the {\it predictive} UDs.

In order to counter start bias effects researchers can fit resource selection function (RSF) models \citep{Manly:2002ay} to telemetry data to investigate what space characteristics an animal might prefer relative to other habitat within its range. A map of space preference based on resource selection is used as a UD approximation than is not subject to start bias because it makes use of spatial habitat characteristics than can be assessed at locations away from where the animal was located during tag deployment. However, \cite{Barnett:2008wj} described some general results concerting the weighted distribution form \cite{Johnson:2008kx, Johnson:2013fk} of a resource selection movement model in discrete time. Assuming a symmetric movement kernel, they show that the limiting distribution is proportional to the resource selection function or the square of the resource selection function depending on how large the movement kernel is relative to the spatial variation of the resource selection function. Although, not explicitly examined, step-selection function (SSF; \citealt{FortinEtAl2005a, avgar2016integrated}) models use this same movement kernel form. So, we conjecture similar results might hold for those types of analyses as well. Therefore, while qualitatively similar, RSFs and UDs can be quite different depending on the scale of movement.   

Predictive UDs seek to blend the habitat association benefits of RSF analysis with movement models such that the resulting UD functions, which appear similar to RSFs, represent the actual UD. Predictive UDs are heuristically what a kernel estimate would look like if the animal was allowed to move through its habitat for a very long (infinite) time. In essence a movement model is fitted to the telemetry data and the location of the animal is predicted from the movement model for a time far in the future. If it exists, this is the stationary or limiting distribution of the movement process. These limiting distributions are independent of the starting location, thus, removing the start bias \citep{whitehead2013inferring, wilsonestimating}. \cite{moorcroft1999home} present an early example of a predictive UD approach modeling home rage of coyotes ({\it Canis latrans}) witha movement model which included a response to scent marks. \cite{wilsonestimating} extended their work to continuous-time Markov chains that included habitat based movement. Although, the CTMC predicted UD was derived numerically from the fitted model components, so, there was no direct parameter association that could be used to relate habitat variables with the UD surface. \cite{michelot2018langevin} used continuous-time and space Langevin diffusions to model resource selection and movement. From this model it is possible to calculate an analytic UD with the exponential form that is used for RSFs, 
\begin{equation}
\label{eq:exp.form}
\bu(\bs) \propto \exp\{\bx(\bs)'\bb\},
\end{equation} 
where $\bu(\bs)$ is the UD at location $\bs$ and $\bx(\bs)$ are habitat covariates at location $\bs$. This model produces the  type of analytic UD prediction that directly relates the UD to habitat variables through the coefficients $\bb$. However. There are some drawbacks for this approach. First the likelihood for this model is not available in closed form, so a discrete time approximation must be use. Second, this model is only applicable to continuously valued covariates because the habitat gradient surfaces, $\nabla \bx(\bs)$, are used in the movement model \citep{michelot2018langevin}. Finally, the Langevin diffusion limiting distribution in (\ref{eq:exp.form}) is only valid when the diffusion component is constant. That is the animal does not change its overall rate movement in different habitats; only the advection component is altered by habitat (See Section 3). 

Herein we consider a general class of CTMC models for which we can obtain closed form predictions of the UD from the fitted model parameters. The CTMC model has several advantages over the Langevin derived UD. First, there is no approximation necessary to calculate the likelihood for optimization. Second, continuous or discrete habitat covariates can be used for movement modeling and UD prediction. Finally, overall movement rate can be influenced by habitat, that is, diffusion rate can be altered by habitat, not just advection. Moreover, because there is no approximation for the likelihood that error does not propagate to the UD prediction, so it is exact. To fit these model, we also developed the {\tt R} package {\tt walk}\footnote{Available from: \url{https://dsjohnson.r-universe.dev/walk}} that can fit general CTMC models including the more general forms requiring numerical calculation of the UD. 


%There is one drawback to the CTMC model if form (\ref{eq:exp.form}) is desired. That is one cannot use gradient surface covariates such as the ocean current or surface wind vector fields that \cite{Johnson353516} used to model fur seal ({\it Callorhinus ursinus}) migration with a CTMC model. This is not a shortcoming of CTMC models, it just cannot be proven that it has exponential form, but the numerical methods. So, in addition to exact UD modeling we will also consider approximations to  Langevin diffusions using CTMCs and a discrete space approximation as opposed to the discrete time approximation of \citep{michelot2018langevin}. To fit these model, we also developed the {\tt R} package {\tt walk}\footnote{Available from: \url{https://dsjohnson.r-universe.dev/walk}} that can fit general CTMC models including the more general forms requiring numerical calculation of the UD. 
...
Here we provide a brief description of continuous-time Markov chain (CTMC) models. More detail can be found in \citet{norris1998markov}, \cite{Johnson353516}, or \cite{Hanks:2015aa}. In a CTMC movement model the geographical study domain is partitioned into a cells or other discrete locations indexed by $\fC = \{1,\dots,n\}$. Each cell, $i$, has a set of neighboring cells $\fN_i = \{j\in \fC: i \sim j\}$, where $i\sim j$ means that an animal can go from cell $i$ to cell $j$ in one move. For example, in a raster partition, neighboring cells might share a border (rook neighborhood) or a corner point(queen neighborhood). Here we assume that if $i\sim j$, then $j\sim i$, that is, movement is symmetric between cells. As with all CTMCs, the complete path of the animal, $P$, can be decomposed into the {\it jump times} at which cell transitions are made, $\btau = \{\tau_0,\tau_1,\dots,\tau_M, \tau_{M+1}\}$, and the {\it embedded Markov chain}, which is the sequence of discrete-space cells visited, $\fG = \{g_0,\dots,g_M\}$. Thus, the CTMC path has the bivariate discrete-time representation $P=\{(\tau_m, g_m):m=0,\dots,M\}$, where $\tau_0 = 0$ and $\tau_{M+1} = T$ is the known end of the telemetry deployment. In a slight abuse of notation, we also use $g_t$ and $\fN_t$ (i.e., indexed by continuous time, $t\in [0,T]$) to represent the location and neighborhood of the animal for any time, that is, $g_t = g_m$ and $\fN_t = \fN_{g_m}$ for $t \in [\tau_m, \tau_{m+1})$. It is apparent which context is being used in a particular situation.
...
\end{equation}
One other useful property of CTMCs is that the residence time in a cell, $r_m = \tau_{m+1}-\tau_{m}$, is distributed,
\[
...
\] 
where ``exp" is the matrix exponential function. The conditional distribution of $[g_t|g_0=i] = \bg_i(t)$ is the $i$th row of $\bG(t)$. The movement process can also de thought of as the solution to 

 Then the movement process can be described as the discrete space diffusion process,
...
\item $b_j$ controls movement to cell $j$ from any cell in $\fN_j$, and
...
\] 
where $\bx_i' = (\bx_{bi}',\bx_{ai}')$ and $\bb = (\bb_b, -\bb_a)$. This is the discrete-space equivalent of the continuous-space predictive UD given by \cite{Barnett:2008wj}. However, there is another specific condition for $c_{ij}$ that also produces a closed exponential form UD, $c_{ij} = d_j-d_i$. That is if the cell pair effects are local differences of cell specific conditions then the UD has the form,
\[
u_i \propto  \exp(b_i - a_i+2d_i),
\] 
(see Appendix A). If $d_i = \bx_{di}'\bb_d$ we get the same log-linear exponential form where  $\bx_i' = (\bx_{bi}',\bx_{ai}', \bx_{di}')$ and $\bb = (\bb_b, -\bb_a, 2\bb_d)$.
...
meta analysis: \cite{johnson2022greater} with {\tt mixmeta} package \cite{sera2019extended}
...
\end{equation}
From this MH acceptance ratio, one can see that a set of sufficient conditions that satisfy $R_m \ge 1$ for all $m = 1,2,\dots$ and all $(i,j)$ such that $i\sim j$ is 
\begin{enumerate}
...
\item \begin{enumerate}
 ...
 \end{enumerate}
\end{enumerate}
If condition 2a. holds then the cell-pair specific movements are symmetric and 
\[
u_i \propto \exp\{b_i-a_i\}
\]
If condition 2b. holds, that is $c_{ij}$ is constructed as a difference between cell indexed terms, and cell-pair specific movement effects are modeled with relative local differences in cell effects, we can still evaluate the UD in the closed exponential form. The term on the right-hand side of (\ref{eq:acc.ratio}) becomes $\exp\{2d_i-2d_j\}$ and can be distributed to the numerator and denominator to produce 
\[
\alpha_i \propto \exp\{b_i-a_i+2d_i\}\Lambda_i
\]
which satisfies the $R_m\ge 1$ condition and is the limiting distribution. 
